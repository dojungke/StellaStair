    ...
    public sealed class SampleInstructions : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            GUI.Box(new Rect(16, 16, 680, 100),
                $"?ÅÌÉú: {phase}\n?¥Îèô Ïπ???MOVE CONFIRM / ATK ???Ä????ATK CONFIRM\n?àÏÉÅ ?ôÌïò ?ºÌï¥??Ï≤¥Î†•Î∞??úÏãú, Move Reset ?¥Îèô Ï∑®ÏÜå, Space ??Ï¢ÖÎ£å", style);
        ...
        private void DrawHealth(TacticalUnit unit, ref float y)
        {
            ...
            GUI.Box(new Rect(16, y, 320, 28),
                $"{label}  HP {unit.CurrentHealth}/{unit.MaxHealth}  ?¥Îèô {unit.RemainingMovement}/{unit.MovementPoints}", style);
            y += 30f;
        }
    }
}
